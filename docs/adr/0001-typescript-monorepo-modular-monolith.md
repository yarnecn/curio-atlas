...
使用 pnpm workspace + Turborepo 管理 TypeScript Monorepo。网站采用 Next.js App Router，小程序采用 Taro + React，API 采用 NestJS REST，worker 采用 NestJS standalone + BullMQ。业务能力在同一 API 内按模块边界组织，共用一个事实数据库；数据库当前由 ADR-0005 确定为 MySQL。
